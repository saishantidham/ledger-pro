...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const D = {
        ...
        dateIn: document.getElementById('receipt-date'), 
        flatBtn: document.getElementById('open-flat-search'),
        flatBtnText: document.getElementById('flat-btn-text'),
        ...
        mFromIn: document.getElementById('month-from'), 
        mToIn: document.getElementById('month-to'), 
        mCalc: document.getElementById('months-calculated'),
        baseTotalCalc: document.getElementById('calculated-base-total'),
    ...
    window.loadHubData = async function() {
        ...
        const { data: rcpts } = await supabaseClient.from('receipts').select('uuid, date, total_amount, serial_no, receipt_no, flat_no').order('created_at', { ascending: false });
        receiptsData = rcpts || [];
        renderCalendar();
    };
    ...
    window.generateReceipt = function(uuid) {
        UX.playClick(); 
        alert(`Navigating to receipt.html?id=${uuid} (Pending Implementation)`);
    };
    ...
    D.mFromIn.addEventListener('change', calculateMonths); 
    D.mToIn.addEventListener('change', calculateMonths);
    ...
    form.addEventListener('submit', async () => {
        ...
        try {
            await DB.updateFlatMaster(currentSelectedFlatNo, {
    ...
    document.getElementById('modal-next-btn').onclick = () => {
        ...
        D.flatBtn.classList.remove('selected');
        D.flatBtnText.textContent = "Select Flat / Owner...";
